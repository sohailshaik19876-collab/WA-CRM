...
- To keep a free/idle Supabase project from pausing due to
  inactivity, point a scheduler at `GET /api/internal/heartbeat`
  (also accepts `POST`) — every 6-12 hours is plenty. It runs one
  minimal read-only query and nothing else; it has no CRM
  functionality. Send the shared secret as `Authorization: Bearer
  <secret>` (`HEARTBEAT_SECRET`, see `.env.local.example`). Returns
  503 until that variable is set, 401 on a missing/wrong secret. See
  [docs/heartbeat.md](./heartbeat.md) for the Vercel Cron config
  (already wired via `vercel.json`) vs. external-scheduler options,
  and how to verify it's actually running.
