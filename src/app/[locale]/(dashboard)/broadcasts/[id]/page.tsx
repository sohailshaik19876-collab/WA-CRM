...
export default function BroadcastDetailPage() {
  ...
  async function handleResume(scope: 'pending' | 'failed') {
    ...
    try {
      ...
      toast.success(
        payload.remaining > 0
          ? tDetail('toastResumeStartedCapped', {
              count: payload.resuming,
              remaining: payload.remaining,
            })
          : tDetail('toastResumeStarted', { count: payload.resuming }),
      );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-card p-4">
          <div className="text-sm">
            <p className="font-medium text-foreground">
              {isStalled ? tDetail('resumeStalledTitle') : tDetail('resumeTitle')}
            </p>
            <p className="mt-0.5 text-muted-foreground">
              {isStalled
                ? tDetail('resumeStalledHint', { count: pendingCount })
                : tDetail('resumeHint', { count: retryableCount })}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {pendingCount > 0 && (
              ...
              >
                ...
                {tDetail('resumePending', { count: pendingCount })}
              </Button>
            )}
            {retryableCount > 0 && (
              <Button
              ...
              >
                ...
                {tDetail('retryFailed', { count: retryableCount })}
              </Button>
            )}
          </div>
        </div>
