...
export async function POST(
  ...
  // Ownership check, account-wide (RLS automations_select: is_account_
  // member(account_id), migration 017) — any account member may
  // duplicate, not just the automation's original creator.
  ...
  const { data: copy, error: copyErr } = await admin
    ...
    .insert({
      ...
      // The duplicator becomes the new copy's author — same convention
      // as before this fix, just sourced from the verified account
      // context instead of a second, separate auth.getUser() call.
      user_id: userId,
